...
export const createServiceListing = createServerFn({ method: "POST" })
  ...
  .validator((data: ServiceListingInput & { publish?: boolean }) => data)
...
export const getServiceListings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data: filters }) => {
    ...
    let query = supabase
      .from("service_listings")
      .select("*, profiles!service_listings_helper_id_fkey(display_name)")
      .eq("status", "active")
...
export const getOffersForListing = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data: listingId }) => {
    ...
    const { data, error } = await supabase
      .from("offers")
      .select("*, profiles!offers_offerer_id_fkey(display_name)")
      .eq("listing_id", listingId)
      .order("amount_cents", { ascending: false });
...
export const getReceivedOffers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      .from("offers")
      .select(
        "*, profiles!offers_offerer_id_fkey(display_name), service_listings(title)",
      )
